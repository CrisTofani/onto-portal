  ...
  <Row>
    ...
    <Col sm={2}>
      <div className="callout callout-primary">
        ...
        <strong>
          {ontologies.length > 1 ? "ontologie" : "ontologia"}
        </strong>
...
export default class OntologiesList extends React.Component {
  ...
  componentWillMount() { this.props.fetchOntList() }

  render() {
    return this.props.hasFetched
      ? (<Container fluid>{ createOntologies(this.props.data) }</Container>)
      : this.props.error ? <Error msg={ontologiesError} /> : null
  }
}
