...
const hasLangValues = (values) => {
  const langValues =
    values.filter(item => Object.prototype.hasOwnProperty.call(item, 'lang'))
...
const defaultLangValue = (lang) => {
  switch (lang) {
  case 'it': return [{ lang: lang, value: 'Non disponibile in italiano' }]
  case 'en': return [{ lang: lang, value: 'Not available for english' }]
  default: return [{ lang: lang, value: 'Not available for '.concat(lang) }]
  }
}
...
const filterLangValues = ([key, values], lang) => {
  ...
  return isEmptyArray(langValue) ?
    [key, defaultLangValue(lang)] : [key, langValue]
}
...
export const mapLangProps = (data, lang) => {
  const filterLangData = Object.entries(data)
    ...
    .reduce((accumulator, pair) =>
      Object.assign(accumulator, { [pair[0]]: pair[1] }), {})
